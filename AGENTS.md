...
- **E2E**: Playwright contra `build + start` con BD aislada determinista (`e2e.sqlite`, recreada por `pretest:e2e`); specs en serie dentro de cada fichero porque comparten estado de BD; además, cada spec debe operar en una combinación cuenta/mes propia para no colisionar con los demás specs, y `workers: 1` fuerza el orden alfabético de ficheros sobre la misma BD — necesario porque algunas specs asertan el balance histórico absoluto de una cuenta (p. ej. `registro-movimientos`), que cualquier movimiento en otro mes contaminaría si otro fichero corriera antes o en paralelo.
- **Diálogos por fila**: los diálogos de acciones de fila (editar/eliminar) se montan a nivel del componente listado (`movement-list.tsx`) con el DTO capturado en estado del cliente, nunca dentro de la propia fila; tras `revalidatePath` la fila puede desaparecer del árbol y un diálogo anclado a ella perdería el estado de `useActionState` (toast y cierre nunca llegarían). El estado vacío del listado también se renderiza dentro del propio `movement-list.tsx` (client), nunca como swap condicional en `page.tsx`: vaciar la vista (p. ej. eliminar el último movimiento visible) desmontaría el listado y con él el diálogo activo.
