...
5. **E2E** (`e2e/edicion-movimientos.spec.ts`, serie dentro del fichero): registra → edita importe → verifica listado/cierre; edita fecha a otro mes → desaparece del listado y aviso de movimiento movido; cambia de cuenta → balance/cierre de destino; elimina → listado y cierre recalculados. Mismas convenciones de 005 (puerto 3100, BD e2e aislada).
