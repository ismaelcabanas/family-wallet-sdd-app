...
- **Puerto +3**: `findById`/`update`/`delete` en `MovementRepository`; `update` sustituye fila + tags en un batch atómico; `delete` físico con cascade de FK (sin borrado lógico, YAGNI).
...
**Storage**: sin cambios — SQLite/Turso vía Drizzle (esquema de 002 intacto; UPDATE/DELETE sobre tablas existentes, sin DDL ni migraciones).
...
| V | Persistencia SQLite con Drizzle | ✅ PASS | Cero DDL/migraciones: UPDATE/DELETE vía Drizzle sobre el esquema versionado de 002; atomicidad con `db.batch` (data-model §2). |
...
│   └── infrastructure/
│       ├── db/
│       │   ├── DrizzleMovementRepository.ts      # AMPLIADO: findById / update (batch) / delete
...
**Structure Decision**: misma estructura por capas concéntricas que 002/005 (`docs/architecture/overview.md`): dominio y aplicación módulos puros; `page.tsx` adaptador fino; Server Actions y UI en `src/infrastructure/primary/`. Tests co-localizados con su SUT (proyectos node/ui de Vitest); e2e en `e2e/` (spec nueva, en serie dentro del fichero por estado compartido de BD). **No se tocan** `schema/`, `drizzle/` ni el seed: la feature opera sobre las tablas existentes.
...
- **ADR 0011 — Edición por reconstrucción validada y borrado físico**: `Movement.recreate` revalida todas las invariantes preservando `id`/`createdAt`; el puerto amplía `findById`/`update` (batch fila + replace de tags)/`delete` físico apoyado en la FK cascade; se rechazan los updates parciales, el borrado lógico y el delete+recreate (research.md §1–§2).
...
| V | SQLite con Drizzle | ✅ PASS | Cero cambios de esquema/migraciones verificado: no hay DDL en el plan; `update` atómico vía `db.batch` (data-model §2). |
