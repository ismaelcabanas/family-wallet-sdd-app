...
**Project Type**: web-app (monolito Next.js, App Router); sin proyectos ni paquetes nuevos; una ruta nueva (`/summary`).
...
| IV | TypeScript Estricto + Zod en fronteras | ✅ PASS | Única frontera nueva: `searchParams.month` de `/summary`, validada con Zod (mismo esquema que `/`). Sin `any`; VO y DTOs tipados. |
...
├── contracts/
│   └── ui-contract.md          # Contrato de la ruta /summary, el panel y el enlace en /
...
flowchart LR
    ...
    subgraph Inbound["Adaptadores inbound — src/app, src/infrastructure/primary"]
        ...
        SummaryPage["/summary page.tsx (server; valida month con Zod)"]
    ...
    U --> Enlace
    U --> MonthSelector
    MonthSelector -- "/summary?month=YYYY-MM" --> SummaryPage
    Enlace -- "/summary?month={mes activo}" --> SummaryPage
    SummaryPage --> UC : "execute(month)"
    ...
    SummaryPage --> Panel : "GlobalMonthlySummaryDTO"
```
